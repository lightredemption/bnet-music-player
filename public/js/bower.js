...
angular.module('ngAudio', []).directive('ngAudio', ['$compile', '$q', 'ngAudio', function ($compile, $q, ngAudio) {
    ...
    this.find = function (id) {
        ...
        localAudioFindingService.find(id).then(deferred.resolve, function () {
    ...
    return function (id) {
        ...
        cleverAudioFindingService.find(id).then(function (nativeAudio) {
            audio = nativeAudio;
...
(function (R, y) {
    ...
    }).info({ angularVersion: "1.6.4" }).directive("ngAnimateSwap", ["$animate", "$rootScope", function (a, b) {
